...
@section('content')
    ...
    @if(session('success'))
    <div class="bg-green-800/80 text-white p-4 mb-6 rounded border border-green-600">
        {{ session('success') }}
    </div>
    @endif
    <!-- Formulario -->
    <div class="max-w-full mx-auto p-4 md:p-6">
        <div class="w-full relative bg-[#112134] rounded-2xl shadow-xl border border-[#d8c495]/20 overflow-hidden pb-2">

        <div class="bg-[#0d1f30] px-6 py-4 border-b-2 border-[#d8c495]/40">
    ...
    <form action="{{ route('avisos.store') }}" method="POST" class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div x-data="usuarioSearch()" class="relative">
                ...
                <div class="relative">
                    <input type="text"
                        ...
                        @blur="cerrarConDelay"
                        autocomplete="off"
                        class="block w-full border border-[#d8c495]/30 rounded-lg pl-3 pr-10 py-2.5 text-white bg-[#0d1f30] focus:outline-none focus:border-[#d8c495] focus:ring-1 focus:ring-[#d8c495] transition-colors placeholder-gray-500"
                ...
                <ul x-show="abierto && resultados.length > 0"
                    x-transition
                    class="absolute z-50 w-full mt-1 bg-[#112134] border border-[#d8c495]/30 rounded-lg shadow-lg overflow-hidden max-h-52 overflow-y-auto custom-scroll"
                    style="display:none">
                    <template x-for="(u, i) in resultados" :key="u.id">
                        <li @mousedown.prevent="seleccionar(i)"
                            :class="i === activo ? 'bg-[#d8c495]/20 text-[#d8c495]' : 'text-white hover:bg-white/10'"
            ...
            <div>
                <label class="block text-xs font-bold text-[#d8c495]/70 uppercase mb-1">Filtrar por Proyecto</label>
                <select name="proyect[]" id="proyect"
                    class="block w-full border border-[#d8c495]/30 rounded-lg px-3 py-2.5 text-white bg-[#0d1f30] focus:outline-none focus:border-[#d8c495] focus:ring-1 focus:ring-[#d8c495] transition-colors custom-scroll cursor-pointer">
        ...
        <div class="flex items-center gap-2 bg-[#d8c495]/10 p-3 rounded-lg border border-[#d8c495]/20">
            <input type="checkbox" name="todos" value="1" id="checkTodos"
                class="w-4 h-4 text-[#d8c495] bg-[#0d1f30] border-[#d8c495]/30 rounded focus:ring-[#d8c495]">
            <label for="checkTodos" class="text-sm font-bold text-[#d8c495] cursor-pointer select-none">
                Enviar este aviso a todos los usuarios (Omite filtro de proyecto)
            </label>
        </div>

        <div>
            <label class="block text-xs font-bold text-[#d8c495]/70 uppercase mb-1">Asunto</label>
        ...
        <div>
            <label class="block text-xs font-bold text-[#d8c495]/70 uppercase mb-1">Mensaje</label>
            <textarea name="mensaje" rows="4"
                class="block w-full border border-[#d8c495]/30 rounded-lg px-3 py-2.5 text-white bg-[#0d1f30] focus:outline-none focus:border-[#d8c495] focus:ring-1 focus:ring-[#d8c495] transition-colors custom-scroll resize-none placeholder-gray-500"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-xs font-bold text-[#d8c495]/70 uppercase mb-2">Canales de envío</label>
                <div class="flex gap-4">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="canales[]" value="interno"
                            class="w-4 h-4 text-[#d8c495] bg-[#0d1f30] border-[#d8c495]/30 rounded focus:ring-[#d8c495]">
                        <span class="ml-2 text-sm text-white">Interno</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                    ...
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="canales[]" value="whatsapp"
                            class="w-4 h-4 text-[#d8c495] bg-[#0d1f30] border-[#d8c495]/30 rounded focus:ring-[#d8c495]">
                        <span class="ml-2 text-sm text-white">WhatsApp</span>
                    </label>
                </div>
            </div>
        </div>

        <div class="pt-4 border-t border-[#d8c495]/20">
            <button type="submit"
                class="w-full md:w-auto md:px-12 float-right bg-[#d8c495] text-[#0d1f30] font-bold uppercase tracking-widest py-3 rounded-lg shadow-md hover:bg-[#c4b385] hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">
                Enviar Aviso
            </button>
        </div>
    </form>
