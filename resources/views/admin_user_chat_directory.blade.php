...
@section('content')
            ...
            @else
                <ul id="userList" class="divide-y divide-carbon border-t border-carbon">
                    @foreach ($users as $user)
                        <li class="user-item py-4 flex items-center justify-between hover:bg-gray-50 transition px-2 rounded-lg -mx-2">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-full bg-gris-carbon text-dorado flex items-center justify-center font-bold text-lg shadow-sm">
    ...
    <div id="adminChatModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center hidden z-50 transition-opacity duration-300">

        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col h-[600px] border border-carbon overflow-hidden transform scale-100 transition-transform duration-300">

            <div class="bg-gris-carbon px-5 py-4 border-b-2 border-dorado flex justify-between items-center shrink-0">
                <div class="flex items-center gap-3">
                    <span class="w-2.5 h-2.5 bg-green-500 rounded-full shadow-[0_0_8px_rgba(34,197,94,0.6)] animate-pulse"></span>
                    <h3 class="text-dorado font-bold uppercase tracking-widest text-sm truncate max-w-[200px]" id="chattingWithName">
                        Chat
                    </h3>
                </div>

                <button id="closeAdminChatBtn" class="text-gray-400 hover:text-white transition focus:outline-none transform hover:rotate-90 duration-300">
            ...
            <div class="p-3 border-t border-carbon bg-white shrink-0">
                <div class="flex items-center gap-2 bg-gray-100 rounded-full px-2 py-2 border border-transparent focus-within:border-dorado focus-within:bg-white focus-within:ring-1 focus-within:ring-dorado transition-all duration-300">
                    ...
                    <button id="sendAdminChatBtn" class="bg-dorado text-white p-2 rounded-full hover:bg-dorado/80 transition-transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0" disabled>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function displayAdminMessages(messages) {
                ...
                if (messages.length === 0) {
                    adminChatMessagesDiv.innerHTML = '<div class="text-center text-gray-400 italic py-4 opacity-70">Inicia la conversación.</div>';
                    return;
                }
                ...
                messages.forEach(message => {
                    ...
                    // Burbuja base
                    messageBubble.classList.add('px-4', 'py-2', 'max-w-[75%]', 'break-words', 'shadow-sm', 'text-sm');

                    if (isUserSender) {
                        // Mensaje RECIBIDO (Usuario): Blanco con texto oscuro
                        messageWrapper.classList.add('justify-start');
                        messageBubble.classList.add('bg-white', 'text-gris-carbon', 'rounded-2xl', 'rounded-bl-none', 'border', 'border-gray-200');
                    } else {
                        // Mensaje ENVIADO (Admin): Gris Carbón con texto Dorado
                        messageWrapper.classList.add('justify-end');
                        messageBubble.classList.add('bg-gris-carbon', 'text-dorado', 'rounded-2xl', 'rounded-br-none', 'font-medium');
                    }
